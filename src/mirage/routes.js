export function routes() {
  ...
  this.post('/categories', (schema, request) =>
    schema.categories.create(JSON.parse(request.requestBody)),
  ) // Create a new category no need to add for now
  this.put('/categories/:id', (schema, request) => {
    // Update a specific category by ID no need to add for now
  ...
  this.del('/categories/:id', (schema, request) =>
    schema.categories.find(request.params.id).destroy(),
  ) // Delete a specific category by ID no need to add for now
  ...
  this.post('/products', (schema, request) =>
    schema.products.create(JSON.parse(request.requestBody)),
  ...
  this.post('/comments', (schema, request) =>
    schema.comments.create(JSON.parse(request.requestBody)),
  ) // Create a new comment
  this.put('/comments/:id', (schema, request) => {
    // Update a specific comment by ID as you like
    let attrs = JSON.parse(request.requestBody)
  ...
  // Allow Firebase requests to pass through MirageJS
  this.passthrough('https://firestore.googleapis.com/**')
  this.passthrough('https://firebaseinstallations.googleapis.com/**')
  this.passthrough('https://fcm.googleapis.com/**')
  ...
  this.passthrough('https://e-commerce-a83c7.firebaseapp.com/**')
  this.passthrough('https://e-commerce-a83c7.appspot.com/**')
  // Add more passthroughs if you use other Firebase services
}
